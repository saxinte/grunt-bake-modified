...
module.exports = function( grunt ) {
	...
	grunt.registerMultiTask( "bake", "Bake templates into a file.", function() {
		...
		var regexRemoveInline = /(?:[ |\t]*<!--\(\s?bake-remove\s+([^>]*)\)-->)\n?([\s\S]+?)(?:[ |\t]*<!--\(\s?bake-end\s?\)-->)/g;
		...
		function parse( fileContent, filePath, values ) {
			...
			fileContent = fileContent.replace( regexRemoveInline, function( match, attributes, content ) {
				return include === true ? "" : inlineReplace( attributes, content, filePath, values );
			});
